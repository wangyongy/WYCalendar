...
/**
 显示规则:优先显示农历节日，其次显示阳历节日，再其次显示节气，最后显示农历日期
 */
- (NSString *)ChineseDate
{
    NSString * result = [self getHoliDayDate];
    
    if (result == nil) result = [self getLunarHoliDayDate];
    
    return result;
}

/**
 不确定日期的节日:父亲节,母亲节,感恩节等

 @param year 年份
 @return 返回父亲节,母亲节,感恩节等在该年份等对应的日期，以字典方式返回
 */
- (NSDictionary *)getUncertainHoliDayDate:(NSInteger)year
{
...
- (NSString *)getHoliDayDate
{
    ...
    [holiday addEntriesFromDictionary:[self getUncertainHoliDayDate:components.year]];
    
    return [holiday objectForKey:[NSString stringWithFormat:@"%zd-%zd",components.month,components.day]];
}
...
/**
 ...
 @return 返回当前日期对应的农历节日
 */
- (NSString *)getLunarHoliDayDate
{
    
    NSDate * nextDayDate = [NSDate dateWithTimeInterval:60*60*24 sinceDate:self];
    
    NSCalendar *localeCalendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierChinese];
    
    NSInteger unitFlags = NSCalendarUnitYear | NSCalendarUnitMonth |  NSCalendarUnitDay;
    
    NSDateComponents *components = [localeCalendar components:unitFlags fromDate:nextDayDate];
    ...
    components = [localeCalendar components:unitFlags fromDate:self];
    ...
    NSString * result = [chineseHoliDay objectForKey:[NSString stringWithFormat:@"%zd-%zd",components.month,components.day]];
    
    if (result == nil) result = [self getSolarTermDate:[[NSCalendar currentCalendar] components:unitFlags fromDate:self]];
    
    if (result == nil) result = [self getChineseDate:components];
...
/**
 ...
 @return 返回当前日期对应节气
 */
- (NSString *)getSolarTermDate:(NSDateComponents *)components
{
    ...
    NSInteger index = (components.year - startYear)*12 + components.month - 1 ;
    
    NSInteger flag = solarTermsDay[index];
    
    NSInteger day;
    
    if(components.day <15) day = 15 - ( (flag>>4) & 0x0f);
    ...
    if(components.day == day){
        ...
        result = (components.month - 1) * 2 + (components.day > 15 ? 1 : 0);
    }
    
    if ( result >= 0  && result < [chineseDays count] ) {
        
        return [chineseDays objectAtIndex:result];
    }
...
/**
 ...
 @param components NSDateComponents
 @return 返回农历日期，每月第一天返回该月份
 */
- (NSString *)getChineseDate:(NSDateComponents *)components
{
    if (components.day == 1) {
        ...
        return [NSString stringWithFormat:@"%@%@",components.isLeapMonth ? @"闰" : @"",chineseMonths[components.month - 1]];
    }
